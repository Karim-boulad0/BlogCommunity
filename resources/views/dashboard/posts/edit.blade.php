                            ...
                            <div class="form-group col-md-12">
                                ...
                                <select name="category_id" id="" class="form-control" required>
                                    ...
                                    @if ($category->title !==null)
                                        <option  @if ($category->id == $posts->category_id)
                                            selected
                                        ...
                                        <div class="tab-pane mt-3 fade @if ($loop->index == 0) show active in @endif"
                                            ...
                                            <div class="form-group mt-3 col-md-12">
                                                ...
                                                <input type="text" name="{{ $key }}[title]" class="form-control"
                                                    placeholder="{{ __('words.title') }}" value="{{$posts->translate($key)->title}}">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>{{ __('words.smallDesc') }}</label>
                                                <textarea name="{{ $key }}[smallDesc]" class="form-control" id="editor" cols="50" rows="10" >{{$posts->translate($key)->smallDesc}}</textarea>
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>{{ __('words.content') }}</label>
                                                <textarea name="{{ $key }}[content]" class="form-control" id="editor" cols="50" rows="10">{{$posts->translate($key)->content}}</textarea>
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>{{ __('words.tags') }}</label>
                                                <textarea name="{{ $key }}[tags]" class="form-control" id="" >{{$posts->translate($key)->tags}}</textarea>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
